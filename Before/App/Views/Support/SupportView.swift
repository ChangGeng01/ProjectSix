...
struct SupportView: View {
            ...
            ZStack {
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 18) {
                        SectionHeader(
                            eyebrow: "Buddy",
                            title: "Pull in a second brain.",
                            subtitle: "Keep it local, small, and low-drama. The point is clarity, not another thread to manage."
                        )
    ...
    private var inboxSummaryPanel: some View {
        PanelCard {
            HStack(spacing: 16) {
                supportCountBlock(title: "Waiting", count: inbox.pendingCount, accent: BeforeTheme.ember)
                supportCountBlock(title: "Heard", count: inbox.heardCount, accent: BeforeTheme.moss)
                supportCountBlock(title: "Archived", count: inbox.archivedCount, accent: BeforeTheme.ink)
            }
        }
    }
    ...
    private func supportCountBlock(title: String, count: Int, accent: Color) -> some View {
        VStack(alignment: .leading, spacing: 6) {
            Text("\(count)")
                .font(.title2.bold())
