...
enum DecisionCapabilityCoverageBuilder {
    ...
    static func sections(
        ...
        let hasTaskGraph = export.runtimeSnapshot.activeTaskGraph?.tasks.isEmpty == false
        let hasTemplates = !(currentBrainState?.activeTemplateIDs.isEmpty ?? true)
        let hasFailureGuards = !(currentBrainState?.failureGuardIDs.isEmpty ?? true)
        let hasSensitiveConstraint = currentBrainState?.activeConstraints.contains(where: { $0.contains("sensitive") }) == true
        ...
        let consistencyCheckedTraceCount = export.recentTraces.filter { $0.consistencyCheck != nil }.count
        let consistencyRejectedTraceCount = export.recentTraces.filter(\.consistencyRejected).count
        let frontLoadShare = average(summary.averagePrefillEquivalentShareByKind.values)

        return [
            runtimeSection(summary: summary, hasSegmentedLatency: hasSegmentedLatency, frontLoadShare: frontLoadShare),
            contextSection(
                summary: summary,
                hasTaskGraph: hasTaskGraph,
            ...
            observabilitySection(summary: summary, brain: brain, flightDeck: flightDeck, hasCalibration: hasCalibration),
            evaluationSection(brain: brain, hasEvolution: hasEvolution, hasCalibration: hasCalibration),
            deliverySection(currentBrainState: currentBrainState)
        ]
    }

    private static func runtimeSection(
        summary: DecisionTestingRuntimeSummary,
        ...
        BASCapabilitySection(
            ...
            items: [
                BASCapabilityItem(
                    ...
                    status: summary.activeProvider == .template ? .partial : .ready,
                    evidence: [
    ...
    private static func contextSection(
        ...
        consistencyRejectedTraceCount: Int
    ) -> BASCapabilitySection {
        BASCapabilitySection(
            domain: .context,
            items: [
                ...
                BASCapabilityItem(
                    ...
                    status: currentBrainState == nil ? .partial : .ready,
                    evidence: ["Brain bootstrap \(currentBrainState == nil ? "missing" : "loaded")"]
                ),
                BASCapabilityItem(
                ...
                BASCapabilityItem(
                    ...
                    status: consistencyCheckedTraceCount > 0 ? .ready : (hasCalibration ? .partial : .missing),
                    evidence: consistencyCheckedTraceCount > 0
                        ? [
                            "Consistency-checked traces \(consistencyCheckedTraceCount)",
                            "Rejected traces \(consistencyRejectedTraceCount)"
                        ]
    ...
    private static func memorySection(
        brain: DecisionTestingBrainSummary,
        currentBrainState: CurrentBrainState?,
        hasTemplates: Bool,
        ...
        BASCapabilitySection(
            ...
            items: [
                ...
                BASCapabilityItem(
                    ...
                    evidence: [
                        "Templates \(currentBrainState?.activeTemplateIDs.count ?? 0)",
                        "Failure guards \(currentBrainState?.failureGuardIDs.count ?? 0)"
                    ]
                ),
                BASCapabilityItem(
        ...
        BASCapabilitySection(
            ...
            items: [
                ...
                BASCapabilityItem(
                    ...
                    status: flightDeck.isPureLocalClosedLoop ? .ready : .partial,
                    evidence: [flightDeck.isPureLocalClosedLoop ? "Current host is pure local." : "Hybrid seam exists but host policy is not local-only."]
                ),
                BASCapabilityItem(
                    ...
                    status: currentBrainState == nil ? .partial : .ready,
                    evidence: ["Brain state \(currentBrainState == nil ? "missing" : "available")"]
                )
            ]
        )
    }
        ...
        BASCapabilitySection(
            ...
            items: [
                BASCapabilityItem(
                    ...
                    status: flightDeck.layerReports.count == BASLayerKind.allCases.count ? .ready : .partial,
                    evidence: ["Layer reports \(flightDeck.layerReports.count)"]
                ),
                BASCapabilityItem(
        ...
        BASCapabilitySection(
            ...
            items: [
                BASCapabilityItem(
                    ...
                    status: currentBrainState == nil ? .partial : .ready,
                    evidence: ["Current brain \(currentBrainState == nil ? "not loaded" : "loaded")"]
                ),
                BASCapabilityItem(
