// MARK: - HostRuntimeCore — chapter 二百八十七 / M774
//
// Phase Alpha 第十三刀(BASHostKit god file 3rd cut, final):从
// HostKitCore.swift 抽出 `BASHostRuntime` 主入口 — 整个 BAS substrate
// 的 host-facing API surface 中心。Phase Alpha 第三个 god file 完结。
//
// 抽出 type:
//   - `BASHostRuntime` (1187 LOC) — 主 host runtime API surface
//     methods: startSession / startReopenSession / lifecycle calls /
//     auditTurn / pre-turn helpers / synthesis policy resolution
//
...
//
// **Phase Alpha milestone**: BASHostKit HostKitCore.swift god file
// 拆完 — 4770 → ~781 LOC across 3 cuts (Synthesis Policy /
// Presentation Configurations / Host Runtime)。CryptoKit
// dependency 保留(SHA256 fingerprint helper for ID generation)。
