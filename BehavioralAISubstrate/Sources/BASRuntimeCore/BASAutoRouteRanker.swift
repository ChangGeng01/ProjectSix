...
public final class BASBpeTokenizerHandle: @unchecked Sendable {
    ...
    // `internal` (was `fileprivate`) so the BPE FFI passthroughs — now in
    // BASAutoRouteRanker+Tokenizer.swift after the god-object extraction
    // (audit ch1040) — can read the raw handle. Visibility-widening only,
    // byte-equal; the pointer is still single-writer (init) / drop-on-deinit.
    internal nonisolated(unsafe) var opaqueHandle:
        OpaquePointer?
