...
public enum BASChapterDoctrineRegistryAllLiterals {
    ...
    // chapter 七百二 第一刀 LEGACY BLOCK — preserved verbatim per
//        「千万不要 删除 只能 commented」 directive。 To restore the
//        Swift literal surface,delete the SQL-backed `all` above
//        and uncomment this block (Swift's `/* */` block comments
//        nest, so the inline `/* ... */` chunks within still parse
//        correctly when this outer wrapper is removed)。
// 
//     // chapter 403
...
//     ]
// 
//     end of chapter 七百二 第一刀 LEGACY BLOCK — see chapter
// open at top of file for the SQL-backed active surface。
}