...
public enum BASEntropyChapterIndex {
    ...
    // chapter 七百二 第三刀 LEGACY BLOCK — preserved per
    // 「千万不要 删除」 directive。 The literal definition
    // below is functionally equivalent to the SQL-backed
    // computed property above (byte-equal via Codable
    // round-trip)。 Per-line `// ` prefix avoids the
    // nested-block-comment trap from string literals
    // containing `/*` patterns。
...
//     ]
    ...
    // chapter 七百二 第三刀 — prePhase2RadicalEntries was
    // a private helper only consumed by the legacy
    // phase2Entries expression。 Since phase2Entries is
    // now SQL-backed,this declaration has no live
    // consumer and is preserved purely as `// `-commented
    // archaeology per 「千万不要 删除」。
