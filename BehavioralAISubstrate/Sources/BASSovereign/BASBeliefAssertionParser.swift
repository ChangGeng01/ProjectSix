...
public enum BASBeliefAssertionParser {
    ...
    // Each pattern's capture group 1 = the asserted value (short, non-greedy, bounded by punctuation /
    // "right" / end). Negative lead-ins ("I'm not sure") don't match (the optional adverb slot won't eat "not").
    ...
    /// Extract the user's asserted value from `turn`, or nil when no explicit assertion frame matches
    /// (⇒ the caller abstains). Pure + deterministic; no model, no network.
