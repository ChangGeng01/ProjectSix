...
public actor BASRoutedEventLogStorage: BASEventLogStorage {
    ...
    private static func eventsArrayViaJsonFfi(
        ...
        if let sid = sessionID {
            ...
            while true {
                guard let page = fetchSessionPage(
                    ...
                    return []   // read error mid-stream → fail closed (no torn history)
                }
                ...
                // ascending); if it somehow can't, that is an anomaly → fail closed.
        ...
        // Timestamp-window read (bounded-preview path, caller supplies the limit) — unchanged.
