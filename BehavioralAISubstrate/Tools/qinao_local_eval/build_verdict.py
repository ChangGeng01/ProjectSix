...
def _load_values(arg):
    ...
    try:
        ...
        sys.stderr.write(f"build_verdict: malformed/truncated values JSON {path}: {e}\n"); sys.exit(2)
...
def evaluate(thr, direction, val, bval, num=None, prov=None):
    ...
    tonum = globals()["num"]  # the module-level float-coercion helper (arg name shadows it)
    ...
    if metric_num in MODEL_CRITICAL and (not isinstance(prov, dict) or prov.get("kind") != "computed"):
        return "ATTEST", f"no computed provenance — recorded literal, not a verified pass: {val}"
...
def build(base, tuned):
    ...
    for (n_, key, cat, direction, thr, level, comp) in METRICS:
        ...
        status, note = evaluate(thr, direction, tval, bval, num=n_, prov=tuned_prov.get(str(n_)))
...
def main():
    ...
    crit_attest = [r for r in verdict["rows"] if r["critical_gate"] and r["status"] == "ATTEST"]
    computed = [r for r in verdict["rows"] if r["status"] in ("PASS", "FAIL")]
    crit_pend = [r for r in verdict["rows"] if r["critical_gate"] and r["status"] in ("PENDING", "NOTE")]
