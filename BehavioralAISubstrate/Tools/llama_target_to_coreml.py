#!/usr/bin/env python3
"""B3 — REAL Llama-3.2-3B-Instruct → stateful Core ML TARGET decoder (head_dim 128) (the ANE half of the ANE∥GPU hybrid).
...
MODEL = sys.argv[1] if len(sys.argv) > 1 else "unsloth/Llama-3.2-3B-Instruct"
...
REVISION = (sys.argv[2] if len(sys.argv) > 2
            else None if MODEL == "unsloth/Llama-3.2-3B-Instruct"
            else None)
...
MAX_SEQ = 512
OUT_DIR = "/tmp/draft"
OUT = f"{OUT_DIR}/LlamaTarget3B_fp16.mlpackage"
