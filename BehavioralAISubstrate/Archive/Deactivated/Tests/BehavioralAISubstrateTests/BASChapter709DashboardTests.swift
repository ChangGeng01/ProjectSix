...
final class BASChapter709DashboardTests: XCTestCase {
    ...
    func testPrintFullAutoRouterDashboard() async throws {
        print("")
        print("## chapter 七百九 第五刀 — comprehensive auto-router dashboard (8 families)")
        print("")
        print(
        ...
        // ----- SOFTMAX -----
        await runSoftmaxRow(dim: 16)
        await runSoftmaxRow(dim: 256)
        await runSoftmaxRow(dim: 1024)

        // ----- LAYERNORM -----
        await runLayerNormRow(dim: 64)
        await runLayerNormRow(dim: 256)
        await runLayerNormRow(dim: 1024)
    ...
    private func runSoftmaxRow(dim: Int) async {
        ...
        let summary = BASBenchmarkHarness.run(
            ...
            let r = BASAutoRouteRanker.softmax(v)
            sink = sink + r.value[0]
        }
        let choice = BASAutoRouteRanker.softmax(v).choice
        printRow(
            workload: "softmax",
            size: "dim=\(dim)",
            winner: choice.rawValue,
    ...
    private func runLayerNormRow(dim: Int) async {
        ...
        let summary = BASBenchmarkHarness.run(
            ...
            let r = BASAutoRouteRanker.layerNorm(v)
            sink = sink + r.value[0]
        }
        let choice = BASAutoRouteRanker.layerNorm(v).choice
        printRow(
            workload: "layer_norm",
            size: "dim=\(dim)",
            winner: choice.rawValue,
    ...
    private func printRow(
        ...
        let nsStr = String(format: "%7.0f", nsPerIter)
        print("| \(w) | \(s) | \(win) | \(nsStr) |")
    }
}
