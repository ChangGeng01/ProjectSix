...
| **4** | Arc-2 single-layer floored caution-withholding (at the P1.5a seam) | medium (opt-in, byte-equal-off) | **Commit 1 ✅** `1cc6dfcb6` (helper `BASDeliberationResolutionCredit` + coordinator slots `evidenceLedger`/`resolvedEvidenceSink`, dormant) · **Commit 2 ✅** `47894bc46` (seam: add `withheldIncrement = max(0, 0.06 − credit)` instead of the constant; byte-equal + **production-inert** since `buildEBrainTurn` does not thread the ledger) · **Commit 3 PENDING** — see below |
| **4·C3** | Activate + prove Step 4 end-to-end | medium | **(a)** thread `evidenceLedger:` through `buildEBrainTurn` (EBrainHostRuntimeSynthesis, mirror `deliberationLoopEnabled`, default nil → byte-equal-off) → reaches the coordinator slot, activating withholding in production. **(b)** e2e consequence test (canonical fixture from `testActivatesAndRefinesViaRealHostRuntimePath`: `.fixtureGeneric`, prompt "Push into an irreversible high-stakes move now.", `.high`): run `off` (medium 0.6427), `on_stuck` (nil ledger → high 0.7027), `on_resolved` (seed `evidenceLedger` with `BASEvidenceAtom`s whose `evidenceKey` == the LIVE keys from `BASDeliberationResolutionCredit.requiredEvidenceKeys(on.decomposeFrame.unknownRecords)`, confidence ≥ 0.3); assert `on_resolved.riskCard.riskLevel == .medium` (back from high), `on_resolved.totalRisk < on_stuck.totalRisk`, `on_resolved.totalRisk >= off.totalRisk` (floored), + replay-determinism + anti-theater (near-miss atom → no withholding). **(c)** flip these rows + add §15 + SCAFFOLD. |
