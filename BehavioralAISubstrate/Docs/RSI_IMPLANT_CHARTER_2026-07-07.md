...
## 全面审计更正批(H1/M2/M3/M5/L 类,2026-07-07 夜)

**H1|J2 锚偏离补注 + 因果声明降格**:R2 预注册写"仪器锚同 J1",执行中被静默丢弃
(新语料上 J1 式锚不可执行,但偏离本身未注记——违纪律,此处补记)。且 R2 与采集
harness 同轮更换(Mac HF 容器 → 设备采集),无锚行隔离 ⇒ **"Mac→设备特征漂移实锤"
降格为"漂移或管线变更,未隔离"**;inc broad 0.638 建立在 16 个负例上,数字本身
统计脆弱。部署向结论(设备特征时代应考虑设备拟合权重)幸存;因果声明不幸存。
**附**:J2 heldout 360 行中 48 行(13.3%)是在位 v2 的训练题(候选零污染)——
不对称偏袒在位,对 REJECTED 判决方向保守(安全),但 0.638 低估、+0.166 低估,
预注册中"对双方公平"一句为误——以本注记更正。

**M2|"统计牙齿"数字性错误更正**:R1 驳回理由中"n 翻倍后 δ=0.01 才有统计牙齿"
错误——CI 半宽 ∝ 1/√n,±0.032→±0.01 需 ~10× n_test 非 2×;J2 在 n_test=360 下
半宽仍 ±0.032(相关性未如期收窄)。教训:预注册效应量门时先算功效,勿拍脑袋。

**M3|轮间多重比较注记**:R1→R2→R3 三轮 + 每轮 12 点网格的族错误率未做校正
(每轮判决独立预注册、无采纳发生 ⇒ 无采纳偏置;但未来某轮 certified 时须在
呈报中携带"这是第 N 次尝试"的完整历史——FSM 收据链已天然携带)。math 容差
−0.01→CI>−0.03 的放宽发生在候选弱点已见之后(有披露、人改、合宪,此处显式注记)。

**M5|R1 预注册 git 见证缺失注记**:R1 的预注册+代码+结果同 commit(f1161aaf4)
——"拟合前冻结"只有叙事担保。自 R2 起已改为预注册先行单独提交;R1 的判决
(人驳)不受影响(驳回不需要预注册保护)。R1 heldout 在 J1 前已当过 v1→v2
裁决集,"只碰一次"实为该数据集第二次承担裁决——注记。

**M6/L 批**:bootstrap 将模板聚簇行视作 iid ⇒ CI 偏窄(分层修正已在 describe3;
聚簇 bootstrap 留 R4);"0.8213 精确复现"实为 4dp 工件精度内一致;评估执法面
...
## R4 结果与暗点1 全弧终判(2026-07-07 深夜)

**J4(纯确认,功效充足:alpha n_neg=86 / math n_neg=193;新语料 572 行 sha 413349cb)**:
| 域 | 在位→候选 | Δ(牙齿) | 判 |
|----|-----------|---------|-----|
| **alpha(主门)** | 0.445→0.592 | **+0.148**,行级[0.056,0.239]∧簇级[0.061,0.257] | **双 CI 清零 = R2 发现 CONFIRMED** |
| math(非劣) | 0.899→0.784 | **−0.115** [−0.175,−0.056] | FAIL(击穿 −0.03 门) |
| overall | 0.811→0.786 | −0.025 [−0.059,+0.008] | 略负 |

**verdict = REJECTED**——但这是**最有信息量的拒绝**:R1/R2/R3 三轮都无法确认的 alpha 效应,
在 R4 用双 bootstrap(行级+簇级同号)证实了 = **环、仪器、四轮纪律全部工作**;候选被驳的
理由是它**是货真价实的权衡而非帕累托改进**(改 alpha 路由 +0.148 的同时劣化 math 路由
−0.115,两方向都有牙齿),不是噪声不是伪影。

**暗点1 全弧终判(R1→R4)**:B2 探针的"设备拟合权重"不是一个待采纳的改进,而是一个
**被四轮纪律逼出真相的科学发现**:*Mac 拟合的在位 v2 在设备特征上是 alpha-弱/math-强,
设备拟合把它翻成 alpha-强/math-弱——这是一条权衡曲线上的另一点,不是更好的点。* 生产
不采纳单一权重。**真正的 levers(留操作员立项)**:①按 purpose 路由不同权重(alpha 类
问题用设备拟合、math 类用在位)②域自适应/混合权重 ③承认 4B 探针的域间容量是固定的、
此消彼长。环的最终价值 = 它没有给出一个假的"改进",而是给出了一条真的权衡曲线 +
一个证实的效应 + 四个诚实负面的判决记录。FSM r4 = rejected(理由入迁移历史);
候选家族三次判决尝试全 rejected,人裁与机器判决全程留痕、可复放。
