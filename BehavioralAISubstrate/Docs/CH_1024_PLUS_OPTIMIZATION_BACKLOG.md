...
| **🟡 Rust crates(rayon multi-threaded)** | PARTIALLY STALE(adjudicated 2026-06-11) | `Cargo/bas-retrieval-ranker` 等 | Original claim("iOS can't reach Rust")conflated dylib with STATIC archive:the umbrella `BASRustMemoryTracker.xcframework` ships ios-arm64 `.a` slices and `bas-retrieval-ranker` is force-linked + LIVE(BASCognitiveBrain+Kernels.swift:28)。 Residual truth:iOS-DEVICE test exercise of the Rust lanes is thinner than Mac `swift test`(endurance probes cover a subset)。 |
...
| 2 | ch 1037.1 | ch 869 `testMPSGraphTimingMediumShape` + 1 sibling(`mps < std×10` upper bound) | 2 | ✅ **DONE** — best-of-3 landed (BASChapter869MPSGraphContestantAndPinsTests.swift:309,394;adjudicated 2026-06-11 audit sweep) |
| 3 | ch 1037.2 | ch 905 `testBenchmarkEventLogAppend100` + `…Append1000`(via `runEventLogBenchmark` helper) | 2 | ✅ **DONE** — best-of-3 in the shared helper (BASChapter905StorePerfBenchmarkTests.swift:174;adjudicated 2026-06-11) |
| 4 | ch 1037.3 | ch 956.6 `testMergeEnginePerf_{singleton,typical8,high32,critic128,pathological512}` + `testMergeApplyPerf_{typical8,high32}`(all `p99 < threshold`) | **7** | ✅ **DONE** — best-of-3 min(p99) landed (BASChapter956_6MergePerfBenchTests.swift:144;adjudicated 2026-06-11) |
...
| 20.1 | ch 1034.1 | **FINDING(ch 1034 discovered):`BASCanonicalKernelInputBuilders.rotaryEmbedding` emits RANK-3 `[seq,heads,headDim]` but `BASMPSGraphRotaryEmbeddingKernel.evaluate` REQUIRES RANK-2 `[seq,headDim]`**(kernel validation line 205-207)。 The builder + kernel each have their own unit tests(`BASCanonicalKernelInputBuildersTests` + `BASMPSGraphRotaryEmbeddingKernelTests`)but were NEVER co-tested — ch 1034's probe is the first call site to feed builder→kernel,exposing the mismatch immediately(`shapeMismatch: "rotaryEmbedding expects rank-2 inputs"`)。 ch 1034 worked around it by hand-rolling the rank-2 input the kernel wants。 | MED — latent substrate integration gap;any caller using the canonical builder for the rotaryEmbedding kernel hits it。 matMul + rmsNorm builders DO match their kernels(verified 3/3)— only rotaryEmbedding is mismatched | Fix options:(a)change `BASCanonicalKernelInputBuilders.rotaryEmbedding` to emit rank-2(if kernel's rank-2 `[seq,headDim]` is the intended contract — likely,since heads fold into seq),OR(b)add a `rotaryEmbeddingRank2(...)` builder + deprecate the rank-3 one,OR(c)make the kernel accept rank-3 + internally reshape。 Add a builder→kernel integration test for ALL 5 kernels to catch sibling mismatches。 Est ~30 LOC + 1 integration test,~1-2 hr。 **✅ DONE ch1034.1(2026-05-29,swift-test-verified):chose option (b)ADDITIVE(red-line-friendly)。 Investigation found the rank-3 builder's ONLY caller is its own unit test(zero production callers)+ the kernel's rank-2 is the sealed M1190 contract(624-commit byte-equal,can't move)。 Added `BASCanonicalKernelInputBuilders.rotaryEmbeddingRank2(input:cosTable:sinTable:sequenceLength:headDim:)` matching the kernel + doc-warned the rank-3 form as future-multi-head-only。 NEW integration test `testRotaryEmbeddingRank2BuilderFeedsKernel`(builder→kernel evaluate,cos=1/sin=0 identity → output==input,verified)。 red-line:existing rank-3 builder/test + kernel UNCHANGED(testRotaryEmbeddingBuilderShapes + testZeroAngleIsIdentity both still pass)。 The "5-kernel integration test" sweep remains a follow-up — matMul + rmsNorm already verified via ch 1034 probe(3/3),only rope needed the new builder;attention + layerNorm have no canonical builder yet。** **✅ SWEEP COMPLETED ch1034.2(2026-05-29,swift-test-verified):added the 2 missing canonical builders `BASCanonicalKernelInputBuilders.attention(q:k:v:seqQ:seqK:dim:)`(rank-2×3)+ `.layerNorm(x:gamma:beta:batch:hidden:)`(rank-2 x + rank-1 gamma/beta)。 NEW integration tests `testCanonicalAttentionBuilderMatchesKernel` + `testCanonicalLayerNormBuilderMatchesKernel` each assert BOTH(a)builder output `==` the file's proven hand-rolled inputs(byte-equal,XCTAssertEqual)AND(b)builder→kernel.evaluate yields the known-correct numeric result(attention zero-QK→[2,3];layerNorm identity→[1,-1,-1,1])。 5/5 kernels now have a builder→kernel integration test guarding the entire mismatch CLASS ch 1034.1 found。 All additive — no kernel/existing-builder change,red-line preserved。** |
