...
**案4 平价·冷机 rider 终判 (2026-07-07 晨,定时任务,手机 5E5C 过夜真冷,先于一切加热负载)**:
新二进制(8 缝 + 案1-5 全上)test-without-building 直跑 sustained 10 分钟,全程 thermal=0(nominal
从未离开——过夜冷机实锤):
- **min00 = 24.7 @ thermal=0** —— 判据 ≥24 命中:晨间冷机基线 24.5 复现(24.7/24.5=+0.8%,
  测量噪声内)。**绝对首窗平价 CLOSED。**
- 平台 min02-10 = 13.6-14.9(中心 14.1-14.2),对文档带 14.2±7% [13.2,15.2] 全在带内;
  min01=15.6 为过渡分钟,方向有利。保持率 57% 形状逐点复现(min02-09 全 55-58%)。
- 608s 全程 PASS,零负载铁律遵守(无 build、无先行设备测试)。

**至此案4 平价证据链三层齐备**:①同机同晚二进制 A/B 逐字节 6528=6528(共模消热);
②冷机 phone-2 平台带内;③过夜真冷同型机 min00 绝对复现 24.5→24.7 + 平台/保持率逐点带内。
**8 缝 + 统一案 1-5 全链:绝对平价 + 相对平价双 CLOSED,无任何回归残疑。** 顺带二次验证
方法教训③:真冷机(过夜)下 nominal 起点即刻复现,当晚 2h+ 冷却永远做不到。
...
**立着的 rider**:①~~07-07 晨任务~~ **DONE(07-07 晨,见上"冷机 rider 终判":min00=24.7
@ thermal=0,24.5 复现,绝对平价 CLOSED)** ②cacheLimit {256,512,768,∞} A/B(独立测量战役)
③token-recycling —— 已跑,诚实负面 CLOSED(TR L1 0.155 vs MTP 0.72,见 memory/账本)。
...
**TOKEN-RECYCLING 判决 (2026-07-06 深夜 — R1 诚实负面,按预注册判据归档,未进 R2)**

R0(成本)PASS:有序 argtop-8 并入单读回,Mac +0.4ms/轮(+1.4%,p1-p5,p4 倒挂=噪声带内);
两臂 a/round 逐提示词全同(0.79/0.62/0.75/0.66/0.83)= 探针观察零扰动。

R1(接受度)FAIL,三重死因:
1. 同行同真值对打:TR L1 命中 0.10-0.22(均值 0.155)vs MTP 同位 0.62-0.83(均值 0.72)——
   替代设计 4.6× 劣势出局。
2. 暖矩阵天花板不够:每代冷启动覆盖仅 36-49%;hit-among-covered ≈0.34 = 跨轮持久矩阵的
   满覆盖乐观上限,仍不过下一条的价。
3. ★揭示偏好定价(决定性,本战役方法学的直接应用):adaptive-K 在同负载上收敛 K=1 =
   产线已实测定价"一行边际 verify > 0.35 条件命中才划算"(MTP L2 的 0.35 都被拒);TR 边际行
   只出 0.155。TR 免的是便宜的起草(头 forward ~1.2ms),免不掉贵的 verify 行——"f≈0" 的
   论纲卖点定错了成本项。链尾免费延伸生态位与替代生态位同死于一张价目表。

普适车道(设计 B)顺带出局:standalone a=0.155 → E[tok]=1.155,verify T=2 行成本即吃掉大半,
且无 BAS 可控的非 Qwen 内核。**复活触发**:①无 MTP 头的主干落到 BAS 可控内核上;②任何真实
负载类(结构化/代码输出?)展示跨轮暖矩阵 L1 ≥0.35(探针一条命令可复测:BAS_TR_PROBE=1)。
资产保留:BASTokenRecyclingMatrix 纯核(已测)+ 探针(env 门,关闭零成本)= 复测仪器现成。

方法注:这是 provider 契约(案4)+ 预注册门(章程)+ 揭示偏好定价(adaptive-K 的 EMA 即价目表)
三件新工具的第一次合用——从立项到诚实负面归档 <2 小时,DFlash 当年走了三个 Gate 数天。
