...
> is the authoritative, reliable headless gate and passes when run with
> `swift test --disable-swift-testing` (**15,015 tests, 101 skipped, 0 failures** on the
> current tree). The separate swift-testing (`@Test`) portion crashes with **SIGBUS
> (signal 10)** under full load in a **headless macOS session**, making the monolithic
> `swift test` exit 1. This is reproducible and independent of the project's code.
...
— a process-level **SIGBUS** in the swift-testing helper subprocess. The XCTest-only gate
finishes clean when Swift Testing is disabled. The monolithic exit code is `1` solely because
of the swift-testing helper crash.
...
| The **recent 5-step arc**'s code (Steps 1–4) | every run | Historical XCTest gate **14,917 / 0 failures**; current XCTest gate **15,015 / 0 failures**. All arc tests are XCTest and pass |
...
1. **XCTest is the authoritative gate** (recommended for headless CI): run
   `swift test --disable-swift-testing`. It bypasses the crashing Swift Testing helper and is
   reliable here — current verified result: **15,015 tests, 101 skipped, 0 failures**.
...
### Note on worker / parallelism flags
...
toolchain (Swift 6.3.2), `--disable-swift-testing` is the reliable XCTest gate. There is still
no SPM flag that runs the full swift-testing engine in one globally serialized helper process —
...
There is no reliable **monolithic** `swift test` invocation for both XCTest and Swift Testing in
this headless environment. For CI/headless, gate on
`swift test --disable-swift-testing` and/or run the `@Test` portion in **`--filter` batches**;
for a fully-green monolithic run, use a **GUI/login session**.

## Reproduction + logs

```
swift test --disable-swift-testing  # PASSES: XCTest headless gate
swift test                          # SIGBUS in swift-testing helper, exit 1
