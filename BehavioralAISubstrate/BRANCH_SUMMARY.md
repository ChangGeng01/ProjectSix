...
| **L8 Rust unification arc — SEALED (extended past initial ch 910 seal)** | **八百九十三-九百十四** | **User directive 「把 L8 统一成 SQL = source of truth + Rust = hot path + Swift = orchestration」。 22 chapters (RFC + 894-914) + ABI 1→17 + 10 Rust modules + ~77 FFI fns + 12 SQL schemas in Rust + 115+ Swift tests + 65 Rust tests pass。 8 stores migrated (LOW + 5 MED + 3 HIGH) + unified facade (904)。 Chapter 905 perf bench: storage-only TIE → DECLINE-WITH-TRIGGER。 Hot-path consolidation proven across **ALL 4 major stores**: vector_index (ch906) **90-134×** + event_log (ch909) **17-102×** + records (ch911) **15-112×** + vault (ch913) **3.75-4.34×**。 Chapter 907 review caught 2 CRITICAL + 8 HIGH + 9 MED + 3 LOW; CRITICAL + 4 HIGH fixed inline + 2 MED in ch908 + 2 MED in ch912 → 5 MED + 3 LOW now registered for future。 Chapter 九百八 byte-eq DEPTH + concurrency stress (first-ever Mutex<Connection> stress-test validation,caught JSON key-ordering finding)。 Chapter 九百十 initial seal + `Docs/L8_ARC_SEAL.md`。 Chapter 九百十四 doc drift fix (CHANGELOG + SEAL + this row extended past initial ch 910 seal per user's「Defer — keep developing」 election)。 ZERO production-default flips (additive only,red-line 7 + ADR-014 OPT-IN preserved)。** |
