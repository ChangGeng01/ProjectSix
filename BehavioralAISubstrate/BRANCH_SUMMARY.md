...
| **L8 Rust unification arc — RE-SEALED (post-全量-审查 fix-sub-arc complete)** | **八百九十三-九百十七** | **User directive 「把 L8 统一成 SQL = source of truth + Rust = hot path + Swift = orchestration」。 24 chapters (RFC + 894-917) + ABI 1→17 (16 bumps,ch 894 starts at 1 not a bump) + 10 Rust modules + ~77 FFI fns + 12 SQL schemas in Rust + 122+ Swift tests + 67 Rust tests pass。 8 stores migrated (LOW + 5 MED + 3 HIGH) + unified facade (ch 904) + 4 hot-path consolidation primitives FLIP-READY across ALL 4 major stores。 Chapter 905 perf bench: storage-only TIE → DECLINE-WITH-TRIGGER。 Chapter 906 vector_index hot-path REAL **90-134×** compute consolidation。 Chapters 909/911/913 event_log/records/vault hot-path FFI-hop reduction (3-110× — honest framing per ch 九百十六 correction;not end-to-end speedup since orch baseline isn't apples-to-apples)。 Chapter 907 review caught issues → ch 908 fixes。 Chapter 九百十四.5 全量 审查 caught 2 CRITICAL + 10 HIGH + 13 MED + 5 LOW in arc 907-914 → ch 九百十五 fixes critical correctness (empty-PK loophole + vault corruption masquerade + real Mutex<Connection> stress test bypassing actor) + ch 九百十六 perf honesty (renamed misleading「speedup」 → ffi-hop-reduction + absolute wall-clock guards) + ch 九百十七 doc drift。 ZERO production-default flips (additive only,red-line 7 + ADR-014 OPT-IN preserved across all 24 chapters)。** |
| **L8 arc 6-pass review cascade — RE-RE-SEALED** | **九百十八-九百二十六** | **Six review passes ran in sequence,each finding real items: ch 918.5 (3rd-pass) → 5C+15H+12M+5L → ch 919 (5 CRITICAL fixes:readFailed enum + sortedKeys + BEGIN IMMEDIATE×4 + UNIQUE constraint + Mutex poison) + ch 920 (HIGH data-model:composite indexes + BLOB cap + wal_autocheckpoint) + ch 921 (HIGH API:L8_ROUTED_OVERVIEW + cosineTopK[Float] overload + partial-conformance asserts)。 ch 921.5 (4th-pass) → 5C+8H+9M+1L → ch 922 (5 CRITICAL:transactional helper + busy_timeout + complete sortedKeys + Rust limit cap + dim/blob validation) + ch 923 (8 HIGH:dedup indexes + BLOB caps + EXPLAIN test + journal_mode verify)。 5th-pass「掘地三尺」 → 1C+5H → ch 924 (CRITICAL TxGuard RAII + 4 HIGH NH1-NH4)。 Test audit found 11 of 18 fixes had ZERO test coverage → ch 925 (delivered 6 of 11 promised tests + 1 stale-pin fix — gap honestly acknowledged in ch 926)。 6th-pass review found **4C+8H+3M** including ch 924 introducing NEW CRITICAL (duplicate index on fresh DB inverting ch 923's 20% write-cost reduction) + ch 925's testWalAutocheckpointIs1000 being fake coverage + 5 missing gap tests + doc drift + fabricated commit numbers → ch 九百二十六 comprehensive fix-of-fix (4 CRITICAL + 8 HIGH all in single commit to break cascade)。 Post-ch-926 state: **133+ Swift tests + 75 Rust unit tests pass** (incl. NEW panic-safety regression guard + schema-migration tests + diagnostic PRAGMA FFI for real verification + shared validateQueryBytes helper across both [UInt8] cosineTopK overloads)。 ABI bumped 17 → 18 (ch 926 added bas_l8_engine_pragma_value_i64 diagnostic helper)。 Production-default flips: still ZERO (additive,red-line 7 preserved across ALL 33 chapters of the arc)。** |
