...
public struct SpeculativeTokenIterator: TokenIteratorProtocol {
    ...
    mutating func speculateRound() {
        ...
        case .rejectionSampling:
            ...
            // target alone (correct for temperature > 0). Mirrors BAS's host-proven `BASSpeculativeRejectionSampler`:
            // accept draft token x with prob min(1, p(x)/q(x)); on rejection emit a draw from the normalized
            // residual (p−q)₊ and stop; if every draft is accepted, emit a bonus token from the trailing main
            // position. DEVICE-CERT-PENDING: the algorithm is proven on host; this MLX port's distribution-
            // equivalence is validated on-device (BAS spec-decode Phase 6). Reached only when the BAS adapter
            // elects `BASSpeculativeMode.sampling`; the upstream/default path above is `.argmaxEquality`.
            // Reproducibility is the caller's responsibility (seed `MLXRandom` before generation).
            var accepted = 0
            var finalToken: MLXArray? = nil
            for i in 0 ..< numDraft {
                let qProbs = softmax(draftLogitsList[i], axis: -1)
                let pLogits = mainLogits[0..., verifyStart + i, 0...]
                let pProbs = softmax(pLogits, axis: -1)
                ...
                if u <= ratio {
                    ...
                    // Resample from the normalized residual (p−q)₊ (fallback to the target if the residual mass
                    // is non-positive — a numerical edge, never a zero vector).
                    let residual = maximum(pProbs - qProbs, 0)
                    let mass = residual.sum().item(Float.self)
                    let resampleLogits = mass > 0 ? residual.log() : pLogits
                    let resampled = MLXRandom.categorical(resampleLogits, axis: -1)
                    eval(resampled)
            ...
            if finalToken == nil {
                // Every draft accepted → bonus token sampled from the trailing main position.
                let bonusLogits = mainLogits[0..., verifyStart + numDraft, 0...]
                let bonus = MLXRandom.categorical(bonusLogits, axis: -1)
                eval(bonus)
