...
| Adapter-trained L2 doctrine | **M343** `BASOrganTrainedWeightProvenance` + `BASOrganTrainedWeightFilter` (4-tier ladder mirroring M295.2 curriculum tier ladder) | EB-1 + EB-2 (external resources required, see `QINAO_EXTERNAL_BOTTLENECKS_BACKLOG.md`) | M67.4 `testPersonaPanelDoesNotPromoteEnvelope` + M343 production tier filter | **AUTHORED as manifesto v8** (chapter 九十一.7 / `QINAO_MANIFESTO_V8_DOCTRINE.md`) — repository-scope doctrine landing complete; full real-world doctrine landing still gated on EB-1 + EB-2 (compute + domain experts), but the typed gate is closed today regardless of when training-pipeline inputs start flowing. |
