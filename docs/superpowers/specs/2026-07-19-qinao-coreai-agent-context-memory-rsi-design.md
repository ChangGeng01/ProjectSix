...
**Status:** Design approved; implementation and production certification remain `REVISE`; controlled-document convergence pending
...
| bounded optional delegation slots | `runtime.semantic-dag` owns the immutable typed slot vocabulary/topology; `runtime.turn-operation` binds a proposal to one slot; K3 alone allocates the winning branch | W1 value-only DAG/turn-operation contracts; no dynamic topology, Agent loop, or scheduler |
| steer/follow-up Input Event disposition | existing L11/L14, ordinary admission, cancellation, and strict-rebase owners produce the exact authorized disposition/target candidate; `runtime.turn-operation` binds their receipts; K3 only performs the expected-head/event-ID/digest CAS, ordering, and stale-head fence | one idempotent EventLog fact referencing the exact semantic/authorization receipts; byte-identical replay returns the same receipt, changed bytes under one ID are corruption; no follow-up queue or K3 semantic decision |
...
| progressive capability-disclosure projection | `state.context-compiler` derives the exact Provider-step view from an execution-plan-bound source owner plus L7 eligibility and current L11/L14/K4 receipts | embedded only in the `CompiledContextDescriptor`; no registry/store; an unmapped metadata/schema owner is ineligible and requires CreateGate or an exact E/A amendment before use |
| source-scoped delivery-gap projection | `runtime.turn-operation`, operating inside the Observe/Replay plane, is the sole projection producer over each exact K3, hash-covered Artifact reference set, spool range, Zone-C receipt range, K4 status root, or publication-journal cursor/root; Adapter/IO only consumes it | process-local bounded delivery only; no durable global cursor/head, delivery manager, or second EventLog; cold recovery reopens the tagged source owners |
| ΩE evolution read set and candidate | the existing L13 LayerCell under `runtime.semantic-dag` produces the immutable proposal/read-set semantic value; for runtime-state targets K3 remains the sole physical stage/activation/append writer after the existing L11/L13/L14/K4 chain; non-runtime targets retain their explicitly mapped certification/cutover owner | Artifact evidence plus existing K3 staging/activation references for runtime-state candidates; code/weight/schema/policy candidates go only through `runtime.certification` and `production.cutover`; an unmapped target is denied and requires CreateGate/ExtensionGate |
| source-mechanism ablation evidence | `runtime.certification` owns the immutable candidate verdict evidence; `production.cutover` alone consumes an eligible sealed verdict for deployment | no admission manager or mutable promotion state; W1 contract/test vocabulary and W6 evidence only |
...
| typed execution-dependency edge roles | `runtime.semantic-dag` owns the immutable edge value/topology; `runtime.turn-operation` binds exact consumed Artifact/receipt references | no causal database, scheduler, or world-causation authority |
| `ClaimSupportMap` | existing L10 LayerCell owns the bounded exact-spool evidence-to-claim edges and per-claim aggregate | embedded in the existing exact-output artifact family; no evidence ledger or second verifier |
| world causal/temporal/association relation claims | the existing L4 LayerCore under `semantics.layercell` authors/version-controls the governed `WorldClaimSet` relation value; `state.snapshot-contracts` and L8 carry only snapshot-bound candidate/provenance references, while K3/EventLog owns only ordering/persistence | L7 eligibility and L10 verification remain mandatory; no sequence-to-cause promotion and no second knowledge graph store |
| causal/explanation presentation projection | the existing L12 LayerCell derives the read-only presentation value from exact DAG/support/relation/receipt roots after L10 verification; `runtime.turn-operation` only reference-composes it, and L11/L14 govern disclosure | disposable content-addressed cache only; no mutable causal trace or explanation writer |
...
### 1.4 Fine-grained capability substitution without dynamic authority

The approved substitution model is **Authority Spine + Capability Mesh**. The fourteen LayerCores, four Physical Kernels, four ControlRings, seven planes, semantic-DAG owner, and all mutable-state owners are fixed. A replaceable component is therefore one mechanism capability behind an existing owner port, never a dynamically replaceable layer, owner, registry, scheduler, compiler, or policy engine.

On iOS, “plug-in” has three exact phases:

1. **build time** — Swift/Rust/C/C++/Metal/Core AI implementation code is compiled into the signed application or its signed process image, while model/profile material is shipped as governed signed data assets;
2. **host composition** — startup injects the closed set of implementations already shipped and certified for that Release tree, rejecting duplicate stable IDs or ambiguous versions;
3. **Attempt binding** — the existing plan owners select only from that closed eligible set and freeze exact content-addressed references before the affected branch can allocate.

Runtime-downloaded executable code, `dlopen`-style extension, last-registration-wins resolution, registration-order selection, and `latest` / default-version lookup are prohibited. A future remote endpoint may expose observations or model/profile descriptions only as untrusted inputs. The current Attempt may bind only an exact identity already admitted by the installed signed envelope; a new endpoint/profile requires the existing certification and production-cutover path for a later Release, and its adapter code must already belong to that signed application Release.

The logical substitution atom realizes one capability contract, for example a main-text Provider, visual specialist, embedding mechanism, FTS lane mechanism, Provider-specific context accountant, native MTP strategy, prompt lookup, deterministic verifier, or one effect adapter operation. Its existing governed descriptor/profile artifacts bind the exact capability ID and semantic version, contract/schema and canonicalization versions, implementation/model material digests, modality and operation, QualityIdentity/StateABI where applicable, context/resource geometry, entitlement/disclosure/capability ceilings, cancellation/query/recovery semantics, certification evidence, known-issue-set digest, and policy/revocation epochs. Several atoms may share one physical binary or asset package for load, signature, and memory efficiency, but they may not share mutable selection authority or hide one atom's failure, route, or version behind another.

Effective authority is always a narrowing intersection of the parent grant ceiling, package-declared capability, exact WorkUnit/branch request, and current L11/L14/K4 decision. An absent or unknown axis denies; no session-wide sticky grant, child proposal, hook, adapter, or package default may add a right omitted by any input. The resulting grant remains bound to the exact Attempt, branch, purpose, materialized input, destination/resource scope, and current epoch vector.

The **Binding Graph** is a read-only composition of those existing owner-owned content-addressed artifacts, not a new graph store or `PluginManager`. The existing `BASSiliconExecutionBinding` remains the single Provider-step binding root: its root contains the mandatory `providerBranchPolicyArtifactID`, and its canonical mapping substructure remains exactly `stepRuleID → model/profile/plan-template/budget Artifact ID`. A materialized Provider request remains a later, separately addressed payload; no per-step “materialization contract” field or second binding wire is smuggled into that owner.

Other mechanism identities extend only their mapped owners:

- Retrieval substitution in v1 is Release-time, not a new runtime profile authority. `runtime.certification` certifies the exact implementation/tree and `production.cutover` fixes one implementation for each enabled canonical `BASSemanticLaneID`; host composition rejects a missing or duplicate implementation. `state.requirement-planner` compiles the existing canonical `BASLaneQuery`, and the bound lane adapter executes it without choosing another adapter. `BASLaneQuery` / `BASLaneResult` gain no ownerless execution-profile field. A future Release that wants two simultaneously eligible implementations for one lane is blocked until a controlled design maps profile authorship, certification, selection, and query/result binding to existing owners. There is no aggregate StateLake owner or `RetrievalLaneRegistry`.
- `state.context-compiler` freezes the exact Provider accounting/tokenizer/template lineage through the existing request/receipt seam; it never discovers or calls a Provider asynchronously.
- Effect substitution reuses the planned `effect.zone-c-saga` owner, its existing `BASEffectAdapterProfile`, and its composition-validated immutable adapter tuple/map. K3's outbox, pending/arm state, and `BASEffectBoundaryPermit` wires remain unchanged and opaque to adapter selection. Before `dispatch_ready`, Zone C derives the one adapter only from the reopened K3-prepared request/tool invocation and freezes that profile plus the signed Release/map identity in its own private row; local arm consumption and the existing terminal receipt revalidate the same profile. Registration order, caller choice, and default fallback are forbidden. The receipt's already planned optional `executionBindingArtifactID` is not repurposed into a new cross-owner binding authority. There is no `EffectAdapterRegistry` outside the sole Zone-C owner.
- verifier and grounding identities remain plan/profile references consumed by L10 and L7; neither mechanism selects itself or promotes its own result.

The next controlled-document convergence must fold fields assigned to still-approved-missing M owners—especially `state.snapshot-contracts`, `runtime.semantic-dag`, and `effect.zone-c-saga`—into their original non-empty CreateGate candidate manifests before first implementation. A field assigned to an already-existing E/A owner, including `state.context-compiler` or `runtime.turn-operation`, instead requires that exact owner's non-empty ExtensionGate manifest. This specification by itself authorizes neither a create nor an extension, and an implementer may not defer a missing first-wire field into an invented later migration.

Every mechanism follows one shared closure shape without a shared manager:

```text
requirement/policy
→ immutable binding
→ exact compiled/materialized input
→ claim/permit
→ at-most-one protected physical invocation
→ typed observed receipt
→ semantic verification
→ commit, reject, or quarantine
```

This at-most-one rule applies to a claimed or externally relevant physical boundary. An owner-certified pure deterministic step interrupted before commit may be recomputed only where the existing recovery matrix explicitly permits it from identical committed inputs, without replenishing budget or reusing an old external permit.

Changing any bound Provider, model, tokenizer, endpoint/accounting lineage, effect adapter, or semantics-affecting mechanism after allocation requires a new admitted Attempt and fresh compilation. Adapter-internal retry, routing, fallback, or model replacement is forbidden. Before allocation, a predeclared signed fallback edge may select another already eligible binding; after allocation, recovery is only continuation of the same provably live identity, query/reconcile/finalize, or terminal closure. Every optional capability slot closes explicitly rather than disappearing silently.

Runtime adaptation may vary only values inside the installed signed envelope—budgets, cadence, already certified plan arms, cache policy, and bounded strategy collapse. A code, weight, schema, contract, authority, or envelope change is only an immutable ΩE proposal followed by independent certification, signed release, and production cutover for future Attempts.
...
### 2.6 Three non-interchangeable causality tracks

Qinao never collapses execution dependency, evidence support, temporal order, statistical association, and world causation into one generic “causal trace.” It uses three separately owned projections.

**Execution Dependency** belongs to the existing `runtime.semantic-dag` contract. Every semantic parent edge has one closed `dependencyRole` such as data consumption, control barrier, authorization, budget, snapshot, verification, effect predecessor, or publication predecessor. The edge binds the exact predecessor receipt, consumed output Artifact, consumer input slot, role, and applicable binding/epoch identity. It proves why Qinao performed or withheld an operation; it does not assert that one represented-world event caused another.

**Claim Support** remains an L10 value embedded in the existing exact-output artifact family. A bounded `ClaimSupportMap` contains two different cardinalities. Every evidence-to-claim edge binds one material final-spool claim span and normalized proposition digest, one exact evidence Artifact/raw span, transformation/verifier receipts, authority, epistemic status, and exactly one edge disposition `supports | contradicts | insufficient | notApplicable`. Separately, every material claim has exactly one aggregate state `supported | contradicted | conflicted | insufficient | notApplicable` plus the canonically ordered edge references that justify it. Supporting and contradicting edges may coexist and then aggregate to `conflicted`; no last-writer, majority, or highest-score rule may erase the conflict. Evidence that a source *states* a causal claim supports attribution to that source; it does not by itself establish the claimed world causation.

**World causal claims** use the existing StateLake entity/relation lane only as snapshot-bound candidate production. L4 alone authors and version-controls world claims; L7 applies hard eligibility/conflict/coverage, and L10 verifies support before any candidate can be adopted or disclosed as established. At minimum the governed relation vocabulary distinguishes temporal predecessor, co-occurrence, association hypothesis, causal hypothesis, supported causal claim, and refuted causal claim. A causal claim binds cause/effect proposition identities, source/provenance, mechanism or governed rule, population/scope/time interval, assumptions and known confounders, epistemic status, supporting/contradicting evidence, and any controlled-intervention or hold-fixed/counterfactual proof. Model-generated counterfactual prose and output sensitivity are hypotheses unless an independently governed causal model or experiment supplies that proof. Sequence, same-session membership, shared project, proximity, and co-occurrence can never promote themselves to causation.

The repository currently violates that target boundary: `BASKnowledgeGraph.swift`, `BASKnowledgeGraphEventExtractor.swift`, `BASUserStateGraphAwareReducer.swift`, and `BASCognitiveOSConvenience.swift` can derive `.causes` from sequence/co-occurrence and allow the resulting synthetic cycle to influence user-state projection. The production reachability also includes `BASSQLiteKnowledgeGraphStorage.swift` persistence/reload and `BASCognitiveOSBuilder.swift` preload/composition. This whole path—not only the extractor—is a W6 incumbent-retirement obligation. Before cutover, the store schema, preload, builder wiring, convenience fold, and reducer must either become an explicitly non-authoritative historical/association projection or be removed from every production-reachable fold. Persisted legacy `.causes` rows are migrated into an explicitly untrusted association/historical class or quarantined; they cannot be preloaded into L4 world claims, L5/L6 user state, or any authoritative reducer. A negative reachability test starts from every legacy causal row/read/preload entry and proves that none can reach an authoritative state transition. Without qualified causal evidence, a relation may not change a user profile or any authoritative state.

The source EventLog HWM and a projector's consumed cursor are different values. K3/EventLog owns the monotonic source sequence/root. For each exact projector identity/version, visibility scope, and source partition, K3 also owns the single committed projection cursor; there is no projector-local timestamp truth or second cursor store. A projector performs a throwing, sequence-bounded read from the last committed consumed sequence through one frozen source HWM, computes and ordinary-puts an immutable projection root, and then asks K3 to compare-and-set the prior cursor while atomically recording `{projector identity/version, scope, source HWM/root, projection root Artifact ID}`. An output put that loses the K3 CAS is an inert orphan; a committed CAS with a lost reply is reopened idempotently. Equal timestamps and clock rollback cannot skip an event. Read/decode failure, partial batch, projection failure, or cursor-CAS conflict cannot advance the cursor or masquerade as an empty result.

The current non-throwing `BASEventLogStorage` reads, `BASSQLiteEventLogStorage` catch-to-`[]` behavior, timestamp cursor in `BASCognitiveOSConvenience.swift`, and the Semantic companion plan's requirement to preserve those non-throwing methods byte-for-byte are controlled-document defects. The convergence plan must replace production projector use with the throwing sequence-bounded read plus K3 cursor/root CAS before W6 cutover; compatibility methods may remain only as non-authoritative views unreachable from a committing projector.

There is no second causal database, mutable explanation ledger, or `CausalTraceManager`. Inside the Observe/Replay plane, the existing L12 LayerCell deterministically derives a visibility-filtered `ExplanationProjection` from the frozen Attempt root, typed DAG edges, exact spool and ClaimSupportMap, owner receipts, and effect/publication outcomes; `runtime.turn-operation` only reference-composes that value. A content-addressed cache is disposable; replay from identical roots must reproduce byte-identical explanation bytes. L11/L14 still govern whether an explanation may reveal a rejected candidate, protected source, Provider detail, or another Workspace's state.

Certification mutation-tests at least: sequential/co-occurring events never yield a causal claim; a synthetic relation cycle cannot alter user state without qualified causal evidence; every semantic parent edge has a role; every material claim has one aggregate support state and every evidence-to-claim edge has one disposition; mixed supporting/contradicting edges remain `conflicted`; equal timestamp/clock rollback loses no event; read failure does not advance the committed projector cursor; model sensitivity alone cannot create a causal edge; an Attempt/generation race invalidates a counterfactual projection; and crash replay re-derives the same explanation.

These are pending controlled-wire deltas, not fields an implementer may improvise from this prose. At this design snapshot, the Runtime companion's `BASSemanticTurnEdge`/`BASSemanticTurnDAG`, the Sovereign companion's `BASExactOutputVerificationPayload`, and the Semantic companion's candidate/relation shapes predate this section. Before their first implementation, controlled-document convergence must add dependency roles plus the optional-slot closure vocabulary to the original `runtime.semantic-dag` M candidate; make L10 produce the edge-and-aggregate `ClaimSupportMap` nested value while the planned `release.spool-publication` owner keeps `BASExactOutputVerificationPayload` as its sole persisted parent; and define the L4 `WorldClaimSet` relation vocabulary as a `semantics.layercell` same-owner extension while L8 retains only candidate/provenance references. Candidate manifests or ExtensionGate manifests, registry/version entries, current/future fixtures, negative mutations, and all affected owner/plan tables change in the same reviewed unit. Until that unit lands, the old companion code blocks are non-executable design debt rather than an alternative wire.

## 3. Main/Sub-Agent Portfolio and Core AI Production Target

### 3.1 Agent roles

The selected main Agent owns no Qinao authority. It is the primary proposal generator for a terminal-answer branch. For the current local/system portfolio, the host chooses Qwen3.5-4B or AFM before exact context compilation through a signed, certified profile. A future exact custom-remote or system-managed-remote-opaque profile may be selectable as main only after its own role/capability/disclosure/cost/recovery certification and sealed release join. The user choice is an input to that profile; it is not an environment-variable route and cannot be changed by an old coordinator.
...
Each Attempt has zero or one main-Provider identity and lineage. Eligible candidates may be compared after admission, but the selected identity/profile/accounting lineage freezes no later than the start of its first exact context compilation and before any main branch allocation. Once a main-Provider branch is allocated, every main-model proposal in the Attempt is bound to exactly that identity. Changing Provider/model/profile or accounting lineage requires a newly admitted Attempt and fresh context compilation; a custom exact remote identity additionally freezes its observable account/region/model revision, while PCC freezes only its app-visible signed opaque profile/cohort and never guesses a hidden account, region, or model revision. Zero main-Provider allocation is valid for deterministic completion, `needsClarification`, `abstain`, `deny`, `quarantine`, or `model unavailable`. The Provider is only a proposal source: even a successful terminal Provider result does not replace L10 verification, L11 risk, or L14 release authority, and “one main Provider” does not mean one physical call or one semantic authority.
...
V1 maximum model-delegation depth is one, measured from the canonical semantic DAG rather than trusted from a model-provided integer; depth zero is valid when no specialist is needed. Only a root main-model `.internalProposal` branch may produce `BASDelegationProposal`. Specialist, grounding, and verifier input/output schemas expose no delegation field; a specialist-to-specialist edge, a nested model delegation, or computed depth greater than one is rejected before K3 allocation. This bound does not prohibit multiple sibling specialists under the same root or ordinary Mission/Objective/WorkUnit nesting whose join semantics are independently governed.

Dynamic delegation does not mutate the immutable DAG topology. Before the Attempt begins, the frozen DAG contains a finite set of optional typed delegation slots. Every slot binds a unique stable `slotID`, unique `slotOrdinal`, canonical slot-contract digest, allowed purpose/output role, parent root, candidate Provider-role set, maximum input/capability attenuation, budget/deadline ceiling, and join obligation. The DAG rejects duplicate IDs or ordinals and freezes a total slot order by canonical `(slotOrdinal, slotID)` encoding. A `BASDelegationProposal` has its own stable proposal ID/digest. From one exact K3 allocation head, the semantic owner derives the complete matching-unoccupied set and selects its first member in that frozen order; callers cannot choose a different matching slot. One K3 allocation-head CAS atomically binds proposal ID/digest to that slot ID and advances the head. A byte-identical retry reopens the same binding, the same proposal ID with another digest is corruption, and a competing head-CAS loser must reopen the committed head and recompute rather than try an arbitrary next slot. Thus event-head order deterministically resolves concurrent distinct proposals, while one proposal can never occupy two slots. An absent match or exhausted fan-out is a deterministic rejection, not permission to append a node or create another scheduler.

At WorkUnit closure, every frozen optional slot appears exactly once in canonical slot order with one exhaustive disposition: `boundTerminal(childBranchRef, childTerminalReceiptArtifactID)`, `provedUnused(k3ZeroAllocationProofArtifactID)`, or `rejectedBeforeAllocation(reason, decisionReceiptArtifactID, k3ZeroAllocationProofArtifactID)`. `provedUnused` and `rejectedBeforeAllocation` are valid only when K3 proves that the slot never bound or allocated a child. Once a child is bound, only an owner-proved terminal receipt satisfying the slot's join obligation can close it through `boundTerminal`; a cancellation request, quarantine request, last-observed receipt, possible-start state, lost reply, or unresolved reconciliation is not terminal and leaves the parent join incomplete. The terminal receipt itself may report completed, failed, or cancellation/quarantine closure only when the exact child owner proves that state terminal. Missing, duplicate, still-live, foreign-Attempt, superseded-child, or caller-defaulted dispositions also make the parent join incomplete. The closed disposition vocabulary and vector canonicalization extend `runtime.semantic-dag`; `runtime.turn-operation` only reference-composes the result, while K3 remains the authoritative branch-allocation/terminal-fact owner. This is not a `DelegationSlotManager` or new mutable slot store.

The existing Qinao semantic, admission, and execution-plan owners—not the model or an adapter—decide whether to allocate a proposed child, which certified Provider profile may satisfy it, the fan-out, deadline, budget, disclosure, and attenuated capability. Fan-out is bounded by the frozen DAG, required coverage, K1 memory/thermal/power admission, the one-local-HeavyPhase rule, BudgetLease, deadline, and certified operating envelopes. Independent CPU retrieval, verification, and remote waits may overlap; multiple local model trunks are not forced to contend merely because several logical Agents exist. A child returns one typed immutable Proposal Artifact. Follow-up work creates a new causally linked WorkUnit or Attempt; it does not create a durable peer mailbox.
...
This is one model-neutral **semantic protocol family**, not one identical wire API, lifecycle, context window, session model, tokenizer, tool surface, or capability set. Each execution-class variant has an exhaustive presence matrix; an adapter may neither turn unavailable fields into `nil`-means-anything nor fabricate defaults for opaque platform facts. It is valid for the eligible set to be empty. Qinao therefore preserves a strong API model's additional certified abilities without granting it additional authority, and it preserves a small local model's bounded role without pretending capability parity.
...
A Provider integration that supports a native pre-tool defer may stop before execution and return the exact tool-use ID/name/input only as a value-only Proposal. Qinao then canonicalizes it and binds exact bytes, purpose, Workspace/WorkUnit/Attempt/generation, causal parents, epochs, budget, capability, and physical-boundary identity before ordinary L10/L11/L13/L14 and K3/K4/Zone-C handling. Validation always applies to the immutable canonical Proposal that would execute. Any normalizer, hook, policy, or adapter rewrite creates a new digest and reruns schema, risk, disclosure, capability, budget, materialization, and current-epoch validation; mutating a previously validated argument object in place is forbidden.
...
For one admitted parallel batch, `runtime.semantic-dag` alone owns immutable membership, causal edges, optional slots, and source order. Each member freezes a plan-owned `concurrencyClass`, side-effect class, canonical sorted conflict set over exact `{ownerID, resource/slot key}`, and proof digest. The execution plan also binds the canonical compatibility-matrix/algorithm Artifact ID, digest, owner, and version; an unknown class pair is incompatible. Two members may overlap only when that frozen algorithm accepts their classes and their conflict sets are pairwise disjoint, or the exact existing owner provides a certified commutativity receipt binding its owner/protocol/version and the exact resource keys. If no mapped commutativity protocol exists, no receipt may be invented and only the disjoint-set rule applies. The executor never infers independence. Every local model trunk carries the same process-wide HeavyPhase conflict key.

The W6 executor uses three phases: two all-member barriers followed by one per-member start gate. First, it invokes the existing owners' pure preparation functions for **all** members: exact governed-mechanism lookup, immutable argument/request preparation, canonical schema validation, and reopening of branch, capability, epoch, cancellation, and block evidence. Second, in canonical source order and before creating a task group, it puts/reopens every invocation/envelope, obtains and equality-checks every required budget-use authorization and existing non-consuming admission receipt named by the frozen plan, proves all concurrency/conflict predicates, and closes any unused preparatory reservation on failure. Any missing, foreign, stale, duplicate, denied, or mismatched member starts zero mechanisms. Neither barrier authorizes an external call or creates a cross-owner atomic transaction.

Only after both all-member barriers pass may the executor create the task group. Immediately before each physical start or protected handoff, that member again reopens its exact immutable digest, cancellation, generation/policy/deletion/revocation epochs, deadline, grant, budget receipt, concurrency proof, and K1 lease. A local Provider branch then wins its fresh K3 claim at the existing start gate. An isolated/remote Provider branch does not use that local ordering: it follows the exact dormant K3 prepare → K4 anchor → K3 arm sequence, wins the fresh Provider claim only at the canonical post-arm point, and then performs the existing handoff. A post-barrier drift, denied recheck, expired lease, or CAS loser executes no physical mechanism. Effect/publication nodes are never generalized into this task group: model branches may produce only value-only effect/tool proposals, while actual effect/publication siblings remain completely outside the group and follow their separate exact K3/K4/Zone-C or journal paths. K2 still supervises Provider work. The executor dispatches ready nodes but cannot choose topology, semantic adoption, Provider identity, or truth.

Completion-order events are a non-authoritative progress projection. Causal tool/proposal results are joined in frozen DAG/source order. The executor drains or quiesces every started child: members with owner-proved terminal closure produce typed result/failure, while `sent_or_unknown`, effect/publication ambiguity, and other owner-defined indeterminate states retain a typed nonterminal reconciliation disposition and block or remand every required join. A thrown hook or transport error becomes an ordinary failure only when the relevant owner proves no protected boundary could have started; it cannot manufacture terminal truth.

A user steer or follow-up has a unique Input Event ID and binds the expected Workspace, WorkUnit, active Attempt, generation, and K3 event head. Existing L11/L14, ordinary admission, cancellation, and strict-rebase owners first produce the exact authorized disposition/target candidate. `runtime.turn-operation` binds those receipts, while K3 only performs the expected-head/event-ID/input-digest CAS, ordering, and stale-head fence and persists exactly one mutually exclusive disposition: `acceptedCurrent`, `staleRejected`, `relocalizedAccepted`, `cancelRequested`, `durablyDeferred`, `strictRebaseRequested`, `independentWorkUnitAccepted`, or `denied`. K3 may derive `staleRejected` from its head mismatch but does not decide the semantic meaning of any other case. Every accepted or relocalized target and its authorizing receipt are explicit. Repeating the same event ID with byte-identical canonical input returns the same committed disposition/target digest; the same ID with different bytes is corruption. A lost reply therefore cannot create another cancel, rebase, or WorkUnit.

A cancellation request is not terminal cancellation. Strict rebase remains illegal until every predecessor boundary is terminal; an unresolved possible-start boundary permits only same-operation reconciliation or a separately authorized independent WorkUnit, never a disguised replacement. Durable deferral is an EventLog fact reopened at the next execution opportunity, not a process-local queue. No disposition may splice bytes into a frozen context, mutate a validated batch, inject a late tool result, or create a hidden `FollowUpQueue`, `AgentLoopManager`, or second scheduler.
...
Durable recovery truth comes only from authenticated, integrity-covered rows, receipts, and roots committed by their exact authoritative domain owners. Artifact bytes count as recovery evidence only when a current or historical owner row/receipt references and hash-covers them; an ordinary-put proposal/result without the K3 terminal-seal row remains an unreachable orphan and authorizes nothing. Stale, earlier-boot, expired, or superseded committed evidence is still trusted as historical fact and then classified explicitly; currentness gates continuation, adoption, disclosure, and mutation rather than erasing the evidence needed to prove staleness.

This does **not** mean every unfinished step is discarded or every committed step is replayable. An uncommitted output is never adopted. A pure step may be recomputed only from committed inputs under the exact same certified implementation, schema/canonicalization, profile, snapshot, and governing epoch identities; otherwise it requires a newly admitted Attempt or remains read-only. An unclaimed branch may advance only under the matrix's exact `safePreclaim` predicates. A same-boot invocation may additionally use positive volatile `sameLiveOwner` and `boundaryUninterrupted` proof to narrow continuation to `liveSameBoundary` or `liveSameInvocation`; such proof never creates durable truth, adoption, or authority. A possible-start or unknown external/Provider/effect/publication boundary is query/reconcile/quarantine-only and is never resent. “Committed” establishes historical truth, not permission to repeat an action.
...
Recovery never serializes raw CoT, Swift stacks/continuations, tasks, actor mailboxes, mutable prompts, credentials, capabilities, one-use permits, Provider thread/session IDs, or volatile model tails. Only when the matrix returns same-boot `safePreclaim` may a deterministic local step interrupted before commit discard orphan output and recompute from committed inputs under the exact implementation/schema/profile/snapshot/epoch identity above, without replenishing budget. An earlier-boot or otherwise stale pure step remains read-only and awaits a separately admitted WorkUnit. A committed K3 terminal-seal row reconstructs its normal `S` without another Provider call; proposal/result Artifacts without that row are unreachable orphans, not an alternate completion. A prepared publication is resumed only through its original journal identity; an armed/unknown sink is lookup/finalize-only. An armed/unknown effect is Zone-C reconcile-only. Policy/deletion/revocation checks happen before touching checkpoint bytes or reacquiring a model.
...
An ΩE background evaluation runs only from a fresh minimum-disclosure `BASContextCapsule`, with no ambient capability and an exact closed allowlist of read-only or proposal-only mechanisms. It produces a candidate Artifact, never a direct memory, skill, prompt, code, weight, or policy write. Its self-ID-free read set is one canonical sorted tagged value. Each entry has the globally unique logical key `(sourceOwnerID, sourceProtocolVersion, logicalSlotID)`. Entries must be in strictly increasing bytewise order of that tuple's schema-defined, typed, length-delimited canonical encoding; a duplicate key, non-increasing order, or two variants for one key is rejected before `readSetRoot` is computed. Every entry begins with that key, then contains exactly one exhaustive `sourceState` variant. `present` binds the expected head `{ArtifactID, digest}`, canonical source-content Artifact ID/digest, sealed snapshot root and HWM, complete generation-vector Artifact ID/digest, policy/deletion/revocation/key epochs, and authority/visibility/erasure scopes. `absent` binds a sealed absence-proof owner/protocol/Artifact ID/digest, the exact snapshot root and HWM at which absence was proved, the complete generation-vector Artifact ID/digest, the same complete epoch/scope fields, and canonical typed `.notPresent` tags for both head and source content. The tags and fixed field order are part of the wire: absent fields are never omitted, encoded as `nil`, empty bytes, zero digests, or caller defaults, and a `present` entry cannot carry an absence proof. No `at least`, omitted “applicable” field, caller-selected subset, or path-only read proof is valid. The root additionally binds one exact target owner/protocol/slot, expected target head-or-absent tagged variant under the same no-omission rule, candidate Artifact ID/digest, purpose/operation identity, and canonical `readSetRoot`.

Each candidate has one mapped semantic target and one already defined physical commit path. For a runtime-state target, L10 reopens and validates the complete value, L11 revalidates risk/confirmation/disclosure, L13 prepares the exact adoption intent, K3 performs invisible staging and equality checks, L14 authorizes only that candidate/read-set root, K4 signs the terminal seal, and K3 alone activates or appends through the canonical Section 9.2 path. The semantic target LayerCore never becomes a durable writer. Every mutable foreign source must be covered by an owner-validated sealed snapshot/version-vector root that K3 can reopen at its activation point. If currentness across the read set cannot be proved atomically through that existing protocol, adoption is denied and a new snapshot/candidate is required—no cross-owner coordinator is created. Drift never edits or force-applies the old candidate. `staged` may be reported only after the exact K3 stage receipt commits, and `adopted`/`success` only after the L14/K4 seal and K3 activation/append receipts commit.

Code, weight, schema, prompt-package, skill-package, or policy candidates are non-runtime targets: K3 may retain only their immutable evidence reference, while `runtime.certification` and `production.cutover` remain their sole adoption/deployment path. They cannot use runtime state commit. A missing semantic target, physical writer, seal chain, or owner-specific protocol is fail-closed and requires CreateGate/ExtensionGate. There is no `BackgroundLearningManager`, `PendingSkillStore`, or `ModelPromotionStore`.
...
Historical/episode retrieval is a frozen L8 projection over one authorized Workspace snapshot, not a search over mutable Provider transcripts. A useful episode result carries the originating goal/opening contract, each hit's bounded anchor neighborhood, the episode's terminal outcome or explicit unfinished tail, exact event-range/source references, and compression/Attempt lineage. For current-truth retrieval, L8 excludes stale or superseded heads while retaining explicit correction/conflict links. An explicit HistoricalReview/as-of/range request instead preserves the requested old heads, corrections, and supersession chain with clear historical labels. Lineage deduplication removes only physically or canonically equivalent projections; it never deduplicates semantically distinct corrections, conflicts, or outcomes. Bounded bookends prevent an isolated matching sentence from masquerading as the task's conclusion. The result distinguishes a verified empty eligible set from partial coverage and retrieval/index unavailability; an unavailable FTS/vector/index lane cannot be reported as “no matching history.” L7 alone decides final eligibility, fusion, conflict preservation, and State Market admission. No `SessionSearchStore`, transcript authority, or second memory ledger is introduced.
...
Capability, tool, and future skill disclosure is metadata-first and progressive. Every metadata candidate must already belong to an exact mapped source owner and bind its owner/protocol version, immutable Artifact/schema digest, purpose, capability class, generation/epochs, and authorization reference; a filesystem name, runtime registration side effect, model claim, or ownerless future skill is ineligible. L8 may retrieve only metadata inside the hard eligibility partition, L7 selects the eligible subset, and the sole L3 `BASContextCompiler` places a compact name/purpose/capability-reference projection in the exact Provider-step context. It includes a full schema or instruction body only for mechanisms already selected before that compilation and fitting the step's budget.

If a preterminal internal Provider step needs expansion after seeing metadata, it must end with a value-only capability-description Proposal bound to one predeclared optional DAG slot. `runtime.turn-operation` only binds that Proposal and the existing eligibility/authorization receipts. L7 and the existing L11/L14 owners determine eligibility/disclosure; L3 performs a fresh compilation containing the exact selected full schema; L2 freezes the destination requirement/plan; K1 admits the required resources; and only then may K3 allocate the winning predeclared Provider-step slot and later issue its ordinary claim. The running context is never mutated in place; an `answerOnly` terminal branch cannot request expansion. Descriptions, retrieved skill text, and model-produced arguments remain escaped inert data, never ambient instructions or capabilities. Immediately before dispatch, L11/L14/K4 and the owning execution seam revalidate the exact current grant, digest, generation, epochs, purpose, and full schema. Disclosure is not authorization. If progressive assembly fails, the system may use only a budget-fitting full projection of the same already authorized set or mark the profile/step ineligible; it cannot widen visibility or fail open at dispatch. This is a projection of existing owners, not a `SkillRegistry`, `ToolCatalog`, `DisclosureManager`, or second context compiler.
...
### 9.4 Live delivery, backpressure, and replay

Live event delivery is an Observe/Replay and Adapter/IO projection, never another EventLog. Replay-required state transitions and terminal outcomes are reconstructed by reopening each exact domain owner: K3 event/boundary rows and hash-covered Artifact references, K4 status when sovereign state is relevant, Zone-C terminal/indeterminate receipts for effects, the independent publication journal for reservation/finalization, and the authorized response spool/visibility rows for visible bytes. No K3 close, Artifact put, or spool state alone implies Zone-C or publication success. Runtime progress, thermal samples, token-rate samples, and other telemetry may be coalesced or sampled under their existing evidence rules. Raw chain-of-thought and private reasoning deltas belong to neither class and are never emitted.

Every live source has a tagged `{sourceOwnerID, sourceStreamID, scope, cursorProtocolVersion}` identity. Attempt-scoped streams additionally require the exact Workspace/WorkUnit/Attempt/generation; process-, device-, Provider-, effect-, or publication-scoped streams carry their own exhaustive scope instead. A delivery-gap projection binds that identity, the last acknowledged cursor, a frozen replay HWM, the exact current source head/root/range at synthesis, and skipped count/range when knowable. There is no synthetic cross-domain cursor, recoverable head, or total order.

For a joined UI projection, `runtime.turn-operation` deterministically derives an `expectedSourceManifest` from the frozen DAG, current K3 branch/boundary references, exact Provider containment, response/visibility plan, effect topology, and publication topology. The multi-source delivery manifest covers every expected source with exactly one tagged variant: `present` with its cursor/HWM, `provedAbsent` with an exact source-owner proof, owner-contract `notApplicable`, or `unavailable/quarantined` with reason and last provable cursor. A missing source or cross-source scope/HWM substitution makes the projection explicitly incomplete and prevents it from presenting command, effect, publication, or response completion. It may still render an authorized partial/gap state. Adapter/IO cannot choose the expected set.

Subscriber queues are bounded and reserve control capacity for detach/gap state. If even that control path cannot enqueue, the `runtime.turn-operation` projection owner atomically marks the subscription detached out of band, accepts no later live event for that subscription, and synthesizes the same gap from the last acknowledged cursor and exact source owner on the next poll/resubscribe. A replay-required event is therefore never silently overwritten. Replay reaches the frozen per-source HWM before live delivery reattaches at its successor; any source drift during replay creates another explicit gap. Coalescible telemetry may replace older samples only while preserving a gap/count/range marker sufficient to prevent false completeness. The UI may resynchronize visible content from the authorized spool and exact domain-owner manifest; it may not infer command truth from missing callbacks. Backpressure may slow or detach an observer, but cannot block an authoritative commit indefinitely or turn an in-memory callback channel into recovery authority.
...
Every review finding is tracked to closure with a stable finding ID, truth/disposition, affected owner and path, severity, implementation wave, exact regression test, negative mutation, closure receipt, and reviewed commit/tree. A numerical statement such as “39” or “45” findings is inventory context, never closure evidence. A true defect closes only when its regression is non-vacuously discovered and passes on the exact candidate tree; a duplicate, superseded design, or false positive remains recorded with an explicit reviewed disposition rather than disappearing. Plan-path grammar, controlled-document references, baseline commits/trees, and test-target/filter membership are machine-checked fields, not prose assertions.

This is a forward gate, not a claim that the current W0 candidate ledger already complies. At this design snapshot, `qinao-review-closure-2026-07-18.json` and `scripts/test_qinao_review_closure.py` still validate the older descriptive field/status shape and do not prove a per-finding regression, negative mutation, or exact candidate tree. W0 therefore cannot claim the 39/45 inventory closed until the closure schema is versioned and migrated. Each finding binds allowlisted regression and mutation test IDs, expected target/discovery cardinality, and a machine-produced test receipt; the aggregate closure receipt binds the candidate commit/tree, runner/manifest digest, complete finding set, and all per-finding receipts. CI rejects an empty/unknown/skipped test ID, zero discovery, stale/different tree, absent mutation, missing receipt, or a record marked fixed without that evidence. The checker validates trusted receipt/manifests rather than executing arbitrary commands from the ledger, and its own mutation suite proves every required field and tree binding is load-bearing.
...
The validation matrix additionally proves the eight cross-domain mechanical laws: Extension isolation, Provider substitution, fused/unfused LayerCell equivalence with an exact fourteen-entry `present | canonicalNoOp` receipt vector, Context exactness, optional-slot/capability closure, state-machine totality, three-track causal closure, and erasure-domain closure. Every physical retrieval lane proves one Release-bound implementation for its exact lane ID plus query/result equality over query Artifact, snapshot, eligibility, and terminal mechanism receipt; no runtime profile choice exists. Every effect fixture proves that the exact K3-prepared request/tool invocation resolves one composition-validated profile, that Zone C freezes/revalidates it across `dispatch_ready` and local arm consumption, and that the terminal receipt reports that same profile without modifying a K3 wire. Removing or substituting one owner-specific binding must fail for the intended owner-specific reason.
...
`runtime.certification` requires ablation evidence for every source-inspired mechanism; this evaluation protocol is not an admission, promotion, or cutover owner. Before a run, the candidate pins the exact baseline, mechanism identity, fixture/corpus/oracle versions, paired seeds, device/OS/Provider cohorts, finite architecturally justified interaction set, run duration, replicate/sample count, metric direction and noise floor, confidence method, non-inferiority bounds, stop/failure rules, and combinations to test. The finite set contains candidate-alone, the full incumbent combination, and predeclared pairwise/high-risk interactions; it does not explore or imply coverage of the unbounded power set. Missing, underpowered, stale, or inconclusive evidence fails certification rather than passing optimistically.

The run records baseline, mechanism-alone, pre-registered interaction combinations, and physical-device long-run/fault-injection results. Authority escape, uncommitted model write, uncertain external resend, stale-Attempt adoption, cross-Provider mutable-state transfer, raw-CoT persistence, silent event loss, and self-promotion are architectural zero-tolerance invariants; the evidence may claim only that observed count was zero over the exact frozen finite matrix, never that testing proved an infinite absence. The comparison measures first-pass verified answer rate, hard-constraint retention, evidence/qualifier coverage, unsupported-claim and omission rates, clarification value, cycle termination, TTFT, verified completion latency, token/context overhead, peak memory, energy, thermal slope, recovery time, and physical-boundary duplicate count. `runtime.certification` may emit a favorable immutable verdict only when the pre-registered confidence/non-inferiority rules prove a Pareto improvement without violating a hard invariant, or when the candidate closes a previously impossible required capability within an explicit budget. Owner Ledger/CreateGate/ExtensionGate rules still govern structural adoption, and only `production.cutover` may consume the sealed certification verdict for deployment; otherwise the candidate remains a non-authoritative lab Artifact.

The reasoning corpus includes multi-constraint ordering, long-text extraction with reversible spans, induced subjective prompts, local overthinking loops, common-sense packing/washing tasks, information-insufficient lateral puzzles, multimodal grounding, lexical/dense conflict, child failure/conflict joins, user steer/cancel, App death, OOM/thermal pressure, Provider loss before and after claim, and local/API model change. An information-insufficient puzzle succeeds by asking or projecting the highest-value admissible clarification, not by inventing a unique answer. Model self-confidence and majority vote are never scoring oracles.

Mechanism-specific mutation and race tests remove the canonical delegation parent, expose delegation in a specialist schema, attempt depth two, exceed adaptive fan-out, mutate arguments after validation, dispatch before both all-member barriers close, join by completion order, steer a stale Attempt, omit or forge a delivery-gap range, omit one required delivery source, mix a cursor/HWM across source scopes, map retrieval unavailability to an empty result, drift one ΩE read-set generation/epoch, report a failed stage as success, and let the executor select adoption without the semantic/K3 receipts. Each mutation must fail for the intended reason and with a non-zero matched test count.
...
|---|---|
| W0 | freeze owners/writers and legacy bypasses; repair non-vacuous gates/CI; converge iOS 27 floors; perform and independently verify the K4 public-framework/entitlement/lifecycle/IPC/helper-private `WAL + FULL` platform spike; pin the approved addendum and external research commits; atomically merge the addendum into the seven controlled documents and map or reject every recovery lifecycle/floor authority; create no production Agent/runtime owner |
| W1 | immutable model-neutral Agent, depth-one delegation, `BASContextCapsule`/continuity/rebase, sufficiency, human-fit, delivery/gap projection, progressive-disclosure metadata, RSI read-set, embedded context/operating envelope, local/system/remote Provider identity/reference, retrieval, memory, semantic-DAG, and NextQuestion value contracts only; no concrete `BASStateABI`, execution behavior, secret, network call, or production activation |
| W2 | one K3 `FULL` nucleus, canonical Workspace/task/Attempt roots and atomic active-head rebase, branch/event heads and the authoritative basis for cursor replay while gap markers remain Observe/Replay projections, preference correction/consent/deletion generations, encrypted content owner, memory/erasure convergence, ΩE read-set staging and mapped-owner CAS evidence, plus K3 corruption/source-root/revocation-fence monotonicity, quarantine, and cold-reopen closure; production model allocation still disabled and durable K4 remains W5 |
| W3 | frozen snapshot/historical episode projections with lineage/anchor/bookends, SQL/exact/FTS/BM25/temporal/entity/dense retrieval, grounding, State Market, metadata-first capability disclosure through the sole exact per-Provider context compiler, and structured reasoning/sufficiency/adaptation fixtures; Provider paths remain shadow-only |
| W4 | external Core AI/AFM Provider packages, execution plans and certified operating envelopes, concrete `BASStateABI`/ProcessorABI, K1/K2/K3 handoff, single-Provider-step pure prepare/currentness-recheck hooks, and prefill/decode/cache mechanisms; the existing unique physical Provider seam may become authoritative only as already planned when called by the incumbent high-level runtime, while replacement Agent graphs/executors and new Core AI/AFM candidates remain shadow/device-validation only—no all-member batch implementation, production batch executor, authoritative new Agent executor, canary, or Core AI model-release cutover |
| W5 | implement durable K4, isolated/remote Provider egress, credential-handle transport, deferred tool-Proposal materialization, Zone C, independent publication journal, effect/publication/query-only uncertain recovery, and erasure mechanisms; migrate the existing Chat Completions/SSE mechanism behind exact materialization/observation without enabling a remote profile; prove all paths with non-zero fixtures and fault injection while production publication remains disabled; retire only direct/synthetic effect paths whose replacement is already closed; K3 nucleus corruption recovery was already a W2 gate |
| W6 | integrate the mechanical semantic-DAG executor, its two all-member barriers/per-member start gate, and the complete replay manifest; in the same sealed cutover satisfy the canonical incumbent-retirement manifest before activating the one new authoritative path; activate the W5 publication path only after that manifest exists; for every profile proposed for production—Core AI, on-device `SystemLanguageModel`, PCC, or other remote—obtain its own equivalent E4, sealed canary/cohort evidence, E5, and separately sealed Release-tree/cohort join; complete eight-window, RSI, delivery-gap, chaos, and device/endpoint certification, and optionally earn an exact-profile local 40/30 claim. Structurally complete remote support may still have zero enabled profiles |
...
The Agent/collaboration mechanisms in this section are W6 post-cutover production invariants. Through W5, the incumbent high-level Agent path remains the sole production semantic/adoption authority and every replacement graph/executor path is contract-only, shadow, or device-validation; this does not prevent the already planned W4 physical Provider seam from serving that incumbent. W6 uses the controlled Runtime plan's canonical incumbent-retirement manifest, not a three-name grep. In the same sealed Release tree, source, link-image, factory, dependency-injection, construction graph, and call-graph gates make every authority-bearing incumbent registry, router, scheduler, dispatcher, fabric runtime, round table, shared-state graph/store, lease manager, state bus, host pipeline/multi-round loop, legacy multi-Provider caller, and every additional symbol enumerated by that manifest unreachable before the mechanical executor activates. There is no migration interval with two production adoption or scheduling authorities.

The executor consumes the exact semantic-layer verification/authorization receipt bundle together with K3-committed branch, budget-use, progress, claim, and ordering receipts. K3 persists references and ordering only; no generic K3 “adoption receipt” is introduced, and the executor never decides proposal adoption itself.
...
The implementation base for a wave is the then-current, explicitly recorded candidate-tree commit after all predecessor-wave commits and receipts, not a branch name or worktree directory. At this document's implementation snapshot, `codex/qinao-w0` pointed to commit `43060b810b3694528690843a9d0281ce12b8df69` / tree `293c668851f6af163680267790978ea663912960`, and the implementation-bearing `codex/qinao-w1` commit `9bc71c98f8986d04cec3c7d3733535039543f3a2` / tree `3f4cc21d9bf1b4d16e6cf060769c24251adc2d6d` was eleven commits ahead; this documentation commit does not redefine that implementation snapshot or substitute for the wave's exact receipt bundle. In-flight staged or untracked changes are not a baseline and cannot enter certification evidence.

The existing controlled-document convergence plan was generated from the older `a82a7bb0…` addendum state and is stale for the decisions in this revision. It must not execute or merge controlled documents until this approved specification is committed as a spec-only immutable commit and a revised version of that same convergence plan pins the exact approved specification commit/blob/digest, updates every generated-from range and review candidate, and mutation-tests that any subsequent specification-byte change invalidates the gate. The order is `spec-only commit → revise/pin the same convergence plan → W0 atomic controlled-document merge`. This is not a new roadmap or W7. A branch name, an earlier green receipt, or a historical non-empty candidate count cannot satisfy that pin.
...
Success may honestly end in `promote`, `deny`, `quarantine`, or `model unavailable`. The architecture, W6 integration, and a base production release can be complete while the 40/30 performance claim remains denied. `cold40`/`sustained30` becomes a release gate only when a product or marketing surface explicitly claims or selects that named performance tier for an exact device/model/material/context/workload/thermal profile; ordinary selection of the underlying production profile does not activate the tier. Failure denies that tier/claim, never the architecture, semantic path, verification floor, or healthy thermal envelope. Performance evidence never waives quality, security, state, or recovery evidence.
...
- trust a model-declared delegation depth, expose delegation in a specialist/verifier schema, or permit a model delegation edge deeper than one;
- persist/share raw CoT, mutable prompt buffers, cross-model KV, or ambient tool handles;
- embed Codex, Pi, Hermes, or Claude's Node/Python/CLI harness as the iPhone runtime, or treat an external harness session/transcript/store as Qinao recovery truth;
- turn a callback stream, completion-order queue, telemetry buffer, or subscriber cursor into a second EventLog or terminal-state authority;
- treat capability/tool/skill metadata disclosure as authorization, persist a competing ToolCatalog/SkillRegistry, or execute arguments mutated after validation without a new digest and complete revalidation;
- allow background/shadow learning to write memory, skills, prompts, code, weights, or policy directly, report an uncommitted stage as success, or adopt without an exact current read-set CAS;
...
- let the W6 executor select semantic truth, Provider identity, proposal adoption, fan-out policy, or topology rather than mechanically dispatch an already authorized DAG and consume K3-committed receipts;
...
- claim that local Core AI, AFM, PCC, and custom APIs have one identical wire/lifecycle/capability set, or encode their incompatible evidence as an ambiguous optional-field bag;
...
The selective Agent-runtime mechanisms above were additionally checked against fixed, clean source snapshots rather than adopted from product descriptions:

- [OpenAI Codex `0fb559f0f6e231a88ac02ea002d3ecd248e2b515`](https://github.com/openai/codex/tree/0fb559f0f6e231a88ac02ea002d3ecd248e2b515): typed correlated lifecycle/event projection, durable-versus-ephemeral delivery, bounded backpressure, expected-turn steer correlation, scoped permission normalization, depth/capacity-limited child configuration, compaction lifecycle, dynamic capability disclosure, and immutable per-turn Provider descriptors; Qinao rejects raw reasoning delivery, mutable session-wide grants, generic Provider retries, runtime Provider registry/config merging, arbitrary child spawning, and any interpretation of the live stream/thread store as K3 truth;
- [Pi `v0.80.10` / `8dc78834cde4e329284cf505f9e3f99763df5529`](https://github.com/earendil-works/pi/tree/8dc78834cde4e329284cf505f9e3f99763df5529): canonical Provider-event terminal grammar, explicit tool-proposal phases, two-phase preparation/execution with every prepare completed before parallel execute, source-ordered result materialization beside completion-order observations, distinct steer/follow-up safe points, immutable parent/active-leaf topology, and structure-preserving compaction cut points; Qinao materializes governed Artifacts and K3 head transitions, adds immutable post-prepare argument/currentness revalidation, and rejects Pi's direct `tool.execute`, permitted post-validation argument mutation, post-tool result rewriting, cooperative abort as execution proof, memory-only queues/leaf, mutable Provider registries, JSONL recovery truth, and automatic transport fallback;
- [Hermes Agent `36f2a966c7f9f69987494b867c3dcf96b69a5766`](https://github.com/NousResearch/hermes-agent/tree/36f2a966c7f9f69987494b867c3dcf96b69a5766): metadata-first skill/tool disclosure, bounded FTS episode retrieval with anchors/bookends/lineage deduplication, and background skill review with prompt/path-level read-before-write discipline; Hermes can write a live skill directly, so Qinao instead converts review into shadow candidate Artifacts, exact mapped-owner read-set CAS, and committed-stage honesty;
- [Claude Agent SDK Python `v0.2.123` / `2d4ef9466427970a6db5e567253bae2b2825010a`](https://github.com/anthropics/claude-agent-sdk-python/tree/2d4ef9466427970a6db5e567253bae2b2825010a): typed subagent attribution, scoped agent definitions, request-correlated tool-permission/hook events, pre-tool `defer`, and fork/lineage value shapes; the SDK launches a bundled closed Claude Code CLI, so Qinao does not infer scheduler, isolation, cancellation, or recovery guarantees from that black box, does not adopt arbitrary executable hooks, and never treats its transcript mirror/session store as authority.

These repositories are design evidence, not runtime dependencies, vendored code, or authority sources. Qinao implements only the independently specified mechanisms behind existing Swift/Rust/SQL/C/C++/Metal/Core AI seams; no external Node, Python, or CLI harness is embedded in the iPhone product.

None of the four source runtimes supplies Qinao's required safety spine: durable possible-start/outcome-unknown classification across K3 prepare/pending, K4 one-shot claim/anchor, K3 arm/handoff, Provider or Zone-C execution, and query/reconcile-only recovery with no blind resend. That protocol remains Qinao-native. A convenient external retry, transcript mirror, completion queue, abort signal, or post-tool callback can never substitute for its permits, receipts, terminal truth, or recovery matrix.
