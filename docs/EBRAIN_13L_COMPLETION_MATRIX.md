...
`L4 / L5 / L6 / L7` 的 `v∞` 白皮书与路线图都属于 `target-state reference`。矩阵仍只陈述当前 repo 真相，不把未来路线折算成现状。
